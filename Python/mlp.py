...
#weight producer
def activate(weights, neuron_inputs):
    if(b==1): # bias existing
...
#forward propagation
def forward_propagate(network, input_data, a_function, *args):
    inputs = input_data
...
def update_weights(network, train_dataset_row, l_rate):
    trained_network=[]
...
def train_network(network, train_dataset, l_rate, n_epoch, n_outputs, a_function, func_derivative, *args):
    final_network=[]
    e = 0.0001 #error-rate 
    sum_prerror =0
    for epoch in range(n_epoch):
        ...
        for row in train_dataset: # last column in row is output value
            ...
            #expected[int(row[-1])] = 1 # tried 10 for a higher rate, its working way better than 1
            ...
            final_network = update_weights(network, row , l_rate)
...
def predict(network, dataset_row, a_function):
    outputs = forward_propagate(network, dataset_row, a_function)
    return outputs #.index(max(outputs))


# Backpropagation Algorithm With Stochastic Gradient Descent
def back_propagation(train_dataset, test_dataset, l_rate, n_epoch, a_function, func_derivative,*n_hidden_nodes):  
    final_network=[]
    bias = b
    ...
    #train_network(network, train_dataset, l_rate, n_epoch, n_outputs, a_function, func_derivative)
    final_network = train_network(network, train_dataset, l_rate, n_epoch, n_outputs, a_function, func_derivative)
    #print('trained network: ',final_network )
    for row in test_dataset:
        prediction = predict(network, row, a_function)
        #normalize:
        #norm = [float(i)/sum(prediction) for i in prediction]
...
#    final_network = back_propagation(train_dataset, test_dataset, l_rate, n_epoch, relu, relu_derivative, 10, 10)
#    
...
#        prediction=predict(final_network, dataset_row, relu)
#        #normalize:
#        #norm = [float(i)/sum(prediction) for i in prediction]
#        index, value = max(enumerate(prediction), key=operator.itemgetter(1))
#        if(index == 0):
...
#    
    # produce json file
    network = init_network(2, 2, 1, 4, 4)
#    d = {}
#    d1 ={}
#    h = 0
#    for i in network:
...
#        l = 0
#        for j in i:
#            #print('Neuron {}.{} {}'.format(h,l,j))
#            l+=1
#            d['Neuron {}.{}'.format(h,l)] = j
#    print(d)
...
#    for i in weights:
#        h +=1
#        dictw['weights{}'.format(h)] = i
#     
#    #print(dictw)
#    #weightlist = {'net_weights:': dictw}
#    print(weightlist)
