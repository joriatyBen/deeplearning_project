...
CHECKPOINT_DIR = 'C:/Users/bajorat_benjamin/Documents/codenv/estimator_cust_chkpts'
...
assert "1.11" <= tf_version, "TensorFlow r1.4 or later is needed"

# Windows users: You only need to change PATH, rest is platform independent
PATH = 'C:/Users/bajorat_benjamin/Documents/codenv/estimator_cust_chkpts'
...
def my_input_fn(file_path, repeat_count=1, shuffle_count=1): #shuffle_count=1
    def decode_csv(line):
...
def my_model_fn(
    ...
    h1 = tf.layers.Dense(4, activation=tf.nn.relu, use_bias=False, trainable=True)(input_layer) #bias_initializer=tf.ones_initializer()
    # Definition of hidden layer: h2 (this is the logits layer)
    # Similar to h1, but takes h1 as input
    h2 = tf.layers.Dense(4, activation=tf.nn.relu, use_bias=False, trainable=True)(h1)

    # Output 'logits' layer is three number = probability distribution
    # between Iris Setosa, Versicolor, and Viginica
    logits = tf.layers.Dense(3, use_bias=False, activation=tf.nn.relu )(h2)
    ...
    optimizer = tf.train.GradientDescentOptimizer(0.01) #AdamOptimizer(0.05) #0.05
...
prediction_input = [[5.1, 3.3, 1.7, 0.5], [5.9, 3.0, 4.2, 1.5],  # -> 1, Iris Versicolor
...
for idx, prediction in enumerate(predict_results):
    ...
    if type == 0:
        tf.logging.info("...I think: {}, is Iris Setosa - > 0".format(prediction_input[idx]))
    elif type == 1:
        tf.logging.info("...I think: {}, is Iris Versicolor -> 1".format(prediction_input[idx]))
    else:
        tf.logging.info("...I think: {}, is Iris Virginica -> 2".format(prediction_input[idx]))




print("foo")
print(tf.train.list_variables(PATH), "=", tf.Variable)
...
#get weights and connect them in one list to export as JSON file
...
for i in names:
    ...
    if(i=='dense/bias'):
        dense_bias.append(classifier.get_variable_value(i))
    elif(i=='dense/kernel'):
        for j in classifier.get_variable_value(i):
            dense_kernel.append(j)
    elif(i== 'dense_1/bias'):
        dense1_bias.append(classifier.get_variable_value(i))
    elif(i== 'dense_1/kernel'):
        for j in classifier.get_variable_value(i):
            dense1_kernel.append(j)
    elif(i== 'dense_2/bias'):
        logits_bias.append(classifier.get_variable_value(i))
    elif(i== 'dense_2/kernel'):
        for j in classifier.get_variable_value(i):
            logits.append(j)

# Matrizen transponieren:
#dense_kernel_transp = [[dense_kernel[j][i] for j in range(len(dense_kernel))] for i in range(len(dense_kernel[0]))]
#dense1_kernel_transp = [[dense1_kernel[j][i] for j in range(len(dense1_kernel))] for i in range(len(dense1_kernel[0]))]
#logits_transp = [[logits[j][i] for j in range(len(logits))] for i in range(len(logits[0]))]

#cumulate lists
weightlist = dense_kernel + dense_bias + dense1_kernel + dense1_bias + logits + logits_bias
#weightlist = dense_kernel_transp + dense_bias + dense1_kernel_transp + dense1_bias + logits_transp + logits_bias
weights = []
...
with open('C:/Users/bajorat_benjamin/Documents/codenv/estimator_cust_chkpts/custom_esti.json', 'w') as outfile:
